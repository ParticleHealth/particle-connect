...
case "$FORMAT" in
  flat)
    ...
    curl -s "https://sandbox.particlehealth.com/api/v2/patients/${PATIENT_ID}/flat" \
      -H "Authorization: Bearer $TOKEN" \
      -H "accept: application/json" | python3 -m json.tool
    ;;
  fhir)
    echo "Retrieving FHIR data..."
    curl -s "https://sandbox.particlehealth.com/api/v2/patients/${PATIENT_ID}/fhir" \
      -H "Authorization: Bearer $TOKEN" \
      -H "accept: application/json" | python3 -m json.tool
    ;;
  ccda)
    echo "Retrieving CCDA data..."
    curl -s "https://sandbox.particlehealth.com/api/v2/patients/${PATIENT_ID}/ccda" \
      -H "Authorization: Bearer $TOKEN" \
