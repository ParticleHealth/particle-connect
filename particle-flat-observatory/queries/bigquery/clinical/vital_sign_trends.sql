...
--              vital_sign_observation_time by stripping it before timestamp parse.
...
parsed_vitals AS (
  SELECT
    ...
    SAFE_CAST(vs.`vital_sign_observation_value` AS FLOAT64) AS observation_value,
    vs.`vital_sign_observation_unit` AS observation_unit,
    -- CRITICAL: Strip leading comma-space before parsing timestamp
    -- Raw values look like: ", 2011-05-28T15:19:11+0000"
    PARSE_TIMESTAMP('%Y-%m-%dT%H:%M:%S%z', LTRIM(vs.`vital_sign_observation_time`, ', ')) AS observation_time,
