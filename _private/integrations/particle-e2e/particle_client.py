"""Particle Health API client for E2E integration.

Handles authentication, patient registration, query submission/polling,
and retrieval of both CCDA (XML/ZIP) and flat (JSON) data formats.
"""
...
class ParticleClient:
    ...
    def get_ccda_data(self, patient_id: str) -> bytes | None:
        """Retrieve CCDA data as a ZIP archive of XML documents."""
        print(f"Retrieving CCDA data for patient {patient_id}...")
        resp = self.http.get(
            f"{self.base_url}/api/v2/patients/{patient_id}/ccda",
            headers=self._headers(accept="application/zip"),
        )
        if resp.status_code == 404:
            print("  No CCDA data available for this patient.")
            return None
        resp.raise_for_status()
        print(f"  CCDA data retrieved ({len(resp.content):,} bytes)")
        return resp.content
