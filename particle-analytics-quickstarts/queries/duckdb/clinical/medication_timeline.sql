...
SELECT
  ...
  CAST("medication_statement_start_time" AS TIMESTAMPTZ) AS start_time,
  CAST("medication_statement_end_time" AS TIMESTAMPTZ) AS end_time,
  CASE
    WHEN "medication_statement_start_time" IS NOT NULL
     AND "medication_statement_end_time" IS NOT NULL
    THEN EXTRACT(DAY FROM
      AGE(
        CAST("medication_statement_end_time" AS TIMESTAMPTZ),
        CAST("medication_statement_start_time" AS TIMESTAMPTZ)
      )
    )
    ELSE NULL
...
ORDER BY
  CAST("medication_statement_start_time" AS TIMESTAMPTZ) DESC NULLS LAST;
