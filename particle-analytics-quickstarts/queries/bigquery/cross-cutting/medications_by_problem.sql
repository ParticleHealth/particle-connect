...
WITH
  ...
  encounter_conditions AS (
    SELECT
      `encounter_id`,
      `encounter_type_name`,
      TRIM(condition_id) AS condition_id,
      `patient_id`
    FROM encounters,
      UNNEST(SPLIT(`condition_id_references`, ',')) AS condition_id
  ...
  encounter_practitioners AS (
    SELECT
      `encounter_id`,
      TRIM(practitioner_role_id) AS practitioner_role_id
    FROM encounters,
      UNNEST(SPLIT(`practitioner_role_id_references`, ',')) AS practitioner_role_id
